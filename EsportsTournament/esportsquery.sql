...
select t.team_name,count(p.player_name) as 'Number of Players'
from teams t
join players p on p.team_id = t.team_id
...
select t.team_name,count(match_id) as 'No of Matches'
from teams t
join matches m on m.winner_id = t.team_id
Group by t.team_name;

/* What is the average salary of players in the teams with country 'USA'?*/
select p.player_name,round(avg(p.salary),0) as 'Average Salary'
from players p
...
select t.team_name,count(m.match_id) as 'Matches Won'
from teams t
